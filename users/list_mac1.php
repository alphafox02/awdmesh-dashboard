...
if ($_POST['xls'] == 'Exportar a xls') {
	header("Location: ../entry/select.php");
	header("Location: list_export.php?tipo=MAC&valor=".$_POST['MAC']."&startdate=".$_POST['startdate']."&enddate=".$_POST['enddate']);
	exit();
}
...
<font style="font-family:'Trebuchet MS',Arial,sans-serif; font-size:28px; color:#0477ad;">Historico Usuarios en la red $display_name</font>
</td><td width=30></td>
<td align='left' width=300><a href='users.php'>Volver</a></td>
</tr></table></h1>
TITLE;

if ($_POST["MAC"] != "Todas") {$sql1 = " AND c_mac='".$_POST["MAC"]."'";} else {$sql1 = "";}
...
if(mysql_num_rows($result)==0) die("<div class=error>No hay registros que coincidan con su seleccion. <a href=\"../users/users.php\">Volver</a></div>");
...
//-----
$node_fields = array("PC" => "c_name","MAC" => "c_mac","Nodo" => "name","Checkin" => "c_time",
...
while($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
    ...
    $i++;
    if ($i > 15000000) {
		$msg = "<br><br>&nbsp;&nbsp;&nbsp;&nbsp;<b><font color='#FF0000'>&iexcl;CONSULTA ABORTADA. HA SUPERADO EL L&Iacute;MITE ONLINE PERMITIDO DE 1500 REGISTROS! <u>LA CONSULTA ES PARCIAL</u>.</b><br><br>";
		break;
	}
}
...
echo "</tr></tr><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td align='right'><b>Descargar &nbsp; <a href='".$protocol."://".$_SERVER['SERVER_NAME']."/". $_SESSION['dashboard'] ."/users/list_export.php?tipo=MAC&valor=".$_POST['MAC']."&startdate=".$_POST['startdate']."&enddate=".$_POST['enddate']."' target='_blank'><img src='../lib/images/excel.ico' width='15' height='15' border='0'></a></td>";
