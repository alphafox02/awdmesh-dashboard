...
<title>CloudController | Network Status</title>
...
if ($_POST["NODE"] != "All") {$sql1 = " AND nodeid='".substr($_POST["NODE"],0,strpos($_POST["NODE"], "-"))."'";} else {$sql1 = "";}
if ($_POST["startdate5"] && (strlen($_POST["startdate5"]) < 12)) {$startdate = rtrim($_POST["startdate5"])." 00:00:00";} else {$startdate = $_POST["startdate5"];}
if ($_POST["enddate5"] && (strlen($_POST["enddate5"]) < 12)) {$enddate = rtrim($_POST["enddate5"])." 23:59:59";} else {$enddate = $_POST["enddate5"];}
...
if(mysql_num_rows($result)==0) die("<div class=error>There are no records that match your selection. <a href=\"../users/users.php\">Volver</a></div>");
$pointCount = mysql_num_rows($result);
$paso = 1 + floor($pointCount/1000); // Comprime grafico si se seleccionan +1000 registros (1000 máximo)

//$strXML will be used to store the entire XML document generated
//Generate the graph element
$strXML = "<graph caption='Activated NODE ".$_POST["NODE"]."' subCaption='' showgridbg='1' lineThickness='1' animation='1' showNames='1' showValues='0' yaxisminvalue='0' numVDivLines='40' formatNumberScale='0' rotateNames='1' areaAlpha='90' showLimits='1' decimalPrecision='1' showAlternateHGridColor='0' divLineDecimalPrecision='0' limitsDecimalPrecision='0' showAreaBorder='0' yAxisName='kbps' xAxisName=' ' >";
$cat = "<categories>";
