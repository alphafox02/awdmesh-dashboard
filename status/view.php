 ...
 */
  function signal_strength_color ($strength){
    if ($strength == "z") {return 'images/greengw.gif';} //solo gateway
...
<div class="note" id="tip">Nodes in <b>bold</b> are gateways. <font style="color:ff3300;">Red text</font> indicates issues. Click headers to sort.
<a href="javascript:close()">hide tip</a></div>
<align="center"><div align="center"><img src="images/refresh_icon.png" alt="" width="20" height="20" align="absmiddle">
<font style='font-family:Arial,sans-serif; font-size:12px; color:#898989;'><b>Information is dynamically updated every five minutes.</b></font></div><br><a href="view_adv.php">Detailed Node Info</a><br><br> 
...
// 
//-----
$node_fields = array("AP Status" => "status", "Node Name" => "name", "Current<br>clients"=>"users", "Down kb" => "kbdown", "Gateway IP" => 'gateway', "Hops to<br>gateway" => "hops", "Mesh<br>latency"=>"RTT", "Last Checkin" => "time");
...
$node_alignment = array("status" => "center", "name"=>"left", "users"=>"center", "kbdown"=>"left", "gateway" =>"left", "hops" =>"center", "RTT" => "center", "time"=> "center");

echo "<table class=\"sortable padded\" border=\"1\" cellpadding=\"5\" cellspacing=\"0\">";
...
while($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
    ...
    if ($row["approval_status"] == "A") {    //show only activated nodes
		...
		if ($value=="status")
            ...
            elseif ($value=="ip") {
            	echo $source."<a href='http://".$row[$value].":8080' target=_blank>".$row[$value]."</a>&nbsp;";
            }
            elseif ($value=="kbdown") {
            ...
            elseif ($value=="uptime")  {
                echo "&nbsp;".$source.$row["uptime"]."&nbsp;";
            }
            elseif ($value=="version")  {
                echo $source.$row["robin"]." / ".$row["batman"];
            }
            else {
