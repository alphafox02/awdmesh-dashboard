...
while ($i < 72) {$data0[$i]= 0; $data1[$i]= 0; $numusers[$i]= 0; $i++;}
...
while($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
	...
	if (($slot-$slotnow) < 0) {
		$data0[($slot-$slotnow)+72] = round($data0[($slot-$slotnow)+72] + ($row['ckbd_hist'] / 1024),2);
		$data1[($slot-$slotnow)+72] = round($data1[($slot-$slotnow)+72] + ($row['ckbu_hist'] / 1024),2);
		$time[($slot-$slotnow)+72] = substr($row["c_time"],11,5);
		if(strpos($macusers[($slot-$slotnow)+72], $row['c_mac'])==false) {
			$macusers[($slot-$slotnow)+72] .= $row['c_mac']." ";
			++$numusers[($slot-$slotnow)+72];
		}
		...
		if(strpos($macusers[($slot-$slotnow)], $row['c_mac'])==false) {
...
?>
        ...
        chart<?php echo $row0['id'];?>.draw(data<?php echo $row0['id'];?>, {width: 500, height: 200, title: '6 Hours | \r \n \r \n \r \n \r \n \r <?php echo " Download: ".number_format($totalkbd,2)."MB | Upload: ".number_format($totalkbu,2)."MB | Clients: ".$totalusers;?>', textStyle: {color: '#000000', fontName: 'Arial', fontSize: 1}, pointSize: '0',lineWidth: '1',colors:['#0084ff','#2f8319','#ffff00'], legend: 'bottom',

			hAxis: {title: '', showTextEvery:'13', direction:'-1', textStyle: {color: '#000000', fontName: 'Arial', fontSize: 10}}
...
?>
        new google.visualization.PieChart(document.getElementById('pie_div<?php echo $row0['id'];?>')).
            draw(datapie<?php echo $row0['id'];?>, {title:"% Usage by Node (MB):"});
      }
