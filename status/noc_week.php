...
while($row0 = mysql_fetch_array($result0, MYSQL_ASSOC)) 
{
	...
	//	if(mysql_num_rows($result)<1) {$texto[$row0["id"]]="No activity data"; } else {$red[$row0["id"]]="1";}
...
while($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
	...
	echo "            datapie".$row0['id'].".setValue(".$i.", 1, ".number_format($row['tkbd']/1024,2).");\n";
	++$i;
}
