...
$time_window = date('Y-m-d H:i:s', strtotime('-7 days'));
$query = "SELECT COUNT(DISTINCT(c_mac)) as coun, SUM(IF(ckbd_hist>0, ckbd_hist, 0))/1024 as down, SUM(IF(ckbu_hist>0, ckbu_hist, 0))/1024 as up, DATE(c_time) as d, HOUR(c_time) as h, FLOOR(MINUTE(c_time)/20) as m, c_time FROM client WHERE netid='".$_SESSION["netid"]."' AND c_time > '$time_window' GROUP BY d,h,m ORDER BY c_time DESC";
...
while($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
	...
	echo "['".date("Y-m-d H:i:s", strtotime($row['c_time']))."',".$row['down'].",".$row['up'].",".$row['coun']."]";
	$down+= $row['down'];
	$up += $row['up'];
	$tu += $row['macs'];
}
...
?>
	...
	if(document.getElementById('chart_div')){
		...
		chart.draw(data, {width: 1000, height: 200, title: 'Usage: Week | <?php echo " Download: ".round($down/1024,2)."GB | Upload: ".round($up/1024,2)."GB | Clients: ".$macrow[0];?>', textStyle: {color: '#000000', fontName: 'Arial', fontSize: 1}, pointSize: '0',lineWidth: '1',colors:['#0084ff','#2f8319','#f3b006'], legend: 'bottom',
		                  hAxis: {title: '', showTextEvery:'100', direction:'-1', textStyle: {color: '#000000', fontName: 'Arial', fontSize: 10}}
		                 });
	      }
	}
    </script>
...
//Selecciona s�lo los ids en client de las MAC con checkin m�s reciente
...
if($currentTime < strtotime($row["c_time"])+604800) {
