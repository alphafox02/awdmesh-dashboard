...
function moveSlider(evnt)
{
	...
	if (mouseover) { // Only if slider is dragged
		...
		display.value = v*60 // put the new value in the slider display element
		return false
	}
	return
}
...
function sliderMouseUp()
{
	if (mouseover) {
		...
		if (slider.yMax == 0) {
			pos = (pos/60 > slider.xMax) ? slider.xMax : pos
			pos = (pos < 0) ? 0 : pos
			carpeLeft(slider.id, pos/60) // Snap horizontal slider to corresponding display position.
		}
		if (slider.xMax == 0) {
	...
	setSliderVal(document.getElementById('slider2').style.left)
	setSliderVal(document.getElementById('slider3').style.left)
	setSliderVal(document.getElementById('slider4').style.left)
...
function StartSlider() // Set up the sliders and the displays on window.onload.
{
