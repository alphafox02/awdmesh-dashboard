    ...
    form.owner_email.style.border='1px solid #7F9DB9';
  	...
  	function localizadireccion(direccion) {
		...
		if (geocoder) {
			...
			geocoder.geocode({address: direccion.value}, function(results) {
				if (results && results.length > 0) {
					var pos = results[0].geometry.location;
					document.addnode_manual.longitude.value = pos.lng(); 
					document.addnode_manual.latitude.value = pos.lat();
				} 			
			}
  		);
		}
