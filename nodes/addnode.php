...
<!--[CDATA[
	...
	function onLoad() 
	{
		...
		*/
		...
		{
			if(markerIsCliked){
				markerIsCliked = false;
				return;
			}
			var point = e.latLng;
			if (point)
			{
				var html = 	'<div style="width:300px; height:auto;"><form name="addnode" method="POST">' +
				'<B>Add Node</B>' +
				...
				'</tr><tr>' +
				  ...
				  '<td><input type="text" size="32"  name="latitude" value="' + point.lat() + '" readonly></td>' +
				'</tr><tr>' +
				  '<td><span class="style1">Longitude:&nbsp;</span></td>' +
				  '<td><input type="text" size="32"  name="longitude" value="' + point.lng() + '" readonly></td>' +
				'</tr><tr>' +
				...
				'<tr><td colspan=2>*Node name cannot begin with #<br>*Enter the MAC format as 11:22:33:44:55:66</td></tr></table></form></div>';
...
echo <<<END
	...
	var marker = new nodeMarker(map, "$net_name", point, "$name", "$notes", "$mac", "$is_gateway", "$gw_qual", "$up", "$draggable", "$users");
END;
	if($utype=='admin'){
